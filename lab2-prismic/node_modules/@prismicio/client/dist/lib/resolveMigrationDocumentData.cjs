...
async function resolveMigrationContentRelationship(relation) {
  ...
  if (relation instanceof Document.PrismicMigrationDocument) {
    return relation.document.id ? { link_type: link.LinkType.Document, id: relation.document.id } : { link_type: link.LinkType.Document };
  }
  if (relation) {
    if (isMigrationValue.contentRelationship(relation.id) || typeof relation.id === "function") {
      return {
    ...
    return { link_type: link.LinkType.Document, id: relation.id };
  }
  return { link_type: link.LinkType.Document };
}
...
const resolveMigrationLinkToMedia = (linkToMedia, migration) => {
  ...
  return { link_type: link.LinkType.Media };
};
